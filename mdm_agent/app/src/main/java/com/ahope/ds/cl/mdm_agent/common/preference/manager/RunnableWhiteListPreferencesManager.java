...
import com.ahope.ds.cl.mdm_agent.common.preference.config.RunnableWhitelistPreferencesConfig;

import java.util.HashSet;
import java.util.Map;

public class RunnableWhiteListPreferencesManager {
	private static RunnableWhiteListPreferencesManager sInstance;
	private final SharedPreferences mPref;

	private RunnableWhiteListPreferencesManager(Context context) {
		mPref = context.getSharedPreferences(RunnableWhitelistPreferencesConfig.PERF_NAME, Context.MODE_PRIVATE);
	}

	public static synchronized void initializeInstance(Context context) {
		if (sInstance == null) {
			sInstance = new RunnableWhiteListPreferencesManager(context);
		}
	}

	public static synchronized RunnableWhiteListPreferencesManager getInstance() {
		if (sInstance == null) {
			return null;
		}
		return sInstance;
